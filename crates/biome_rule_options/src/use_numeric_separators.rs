...
impl DeserializableValidator for NumericLiteralSeparatorOptions {
    fn validate(
        &mut self,
        ...
        if self.minimum_digits.is_some() || self.group_length.is_some() {
            return true;
        }

        ctx.report(
            DeserializationDiagnostic::new(markup! {
                "At least one field among "<Emphasis>"minimumDigits"</Emphasis>" and "
                <Emphasis>"groupLength"</Emphasis>" must be set for "<Emphasis>{name}</Emphasis>"."
            })
            .with_range(range),
        );
        false
